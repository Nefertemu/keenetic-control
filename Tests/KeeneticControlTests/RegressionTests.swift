...
/// Все проверки прежнего SelfTest, разделённые на независимые сценарии.
final class RegressionTests: XCTestCase {
    private var checks = 0
    private var sampleConfig: String { RegressionFixtures.sampleConfig }
    private var groups: [String: FqdnGroup] { RouterConfigParser.parseFqdnGroups(sampleConfig) }
    private var interfaces: [String: KeeneticInterface] { RouterConfigParser.parseConfigInterfaces(sampleConfig) }
    private var liveIfaces: [String: KeeneticInterface] {
        RouterConfigParser.parseInterfaceStatus(json: RegressionFixtures.liveJSON)
    }
    private var routes: [StaticRoute] { StaticRouteParser.parse(config: sampleConfig) }

    private func check(_ name: String, _ actual: String, _ expected: String,
                       file: StaticString = #filePath, line: UInt = #line) {
        checks += 1
        XCTAssertEqual(actual, expected, name, file: file, line: line)
    }
    private func check(_ name: String, _ actual: [String], _ expected: [String],
                       file: StaticString = #filePath, line: UInt = #line) {
        checks += 1
    ...
    override func tearDown() {
        print("Legacy checks: \(checks)")
        super.tearDown()
    }

    func testPunycode() throws {
    ...
    func testInterfaceNames() throws {
    ...
    func testLiveInterfaces() throws {
    ...
    func testLivePingAndDiagnostics() throws {
        let pingCLI = """
        pingcheck:
            profile: vpn
            host: 1.1.1.1
    ...
    func testTextDraft() throws {
        var draft = TextDraft()
        draft.receive("Старое имя")
        check("черновик получает исходное имя", draft.text, "Старое имя")
        draft.text = "Моё новое имя"
        draft.receive("Старое имя")
        check("фоновая проверка не стирает ввод", draft.text, "Моё новое имя")
        draft.receive("Изменено в веб-панели")
        check("внешняя правка не стирает ввод", draft.text, "Моё новое имя")
        check("черновик помнит актуальную конфигурацию", draft.saved, "Изменено в веб-панели")
        draft.reset(to: "Другой туннель")
        check("смена туннеля сбрасывает черновик", draft.text, "Другой туннель")
        draft.receive("Новое имя другого туннеля")
        check("нет правок — имя обновляется автоматически", draft.text, "Новое имя другого туннеля")
        draft.text = ""
        draft.receive("Новое имя другого туннеля")
        check("очистка имени тоже считается правкой", draft.text.isEmpty)
    }

    func testProfileValidation() throws {
        var invalidProfile = RouterProfile()
        check("стандартный профиль валиден", invalidProfile.validationError == nil)
        invalidProfile.port = 0
        check("нулевой SSH-порт отклоняется", invalidProfile.validationError != nil)
        invalidProfile.port = 65536
        check("слишком большой SSH-порт отклоняется", invalidProfile.validationError != nil)
        invalidProfile.port = 22
        invalidProfile.host = ""
        invalidProfile.webURL = "https://router.example/"
        check("веб-адрес не заменяет пустой SSH-адрес", invalidProfile.validationError != nil)
        invalidProfile.transport = .http
        check("HTTP не требует отдельного SSH-адреса", invalidProfile.validationError == nil)
        invalidProfile.webURL = "ftp://router.example/"
        check("неподдерживаемая схема отклоняется", invalidProfile.validationError != nil)
        invalidProfile.webURL = "http://router.example:70000/"
        check("неправильный HTTP-порт отклоняется", invalidProfile.validationError != nil)
        invalidProfile.webURL = "https://router.example/"
        invalidProfile.user = "  "
        check("пустой логин отклоняется", invalidProfile.validationError != nil)
        invalidProfile.user = "admin"
        invalidProfile.transport = .ssh
        invalidProfile.host = "https://router.example/"
        check("URL вместо SSH-узла отклоняется", invalidProfile.validationError != nil)
        invalidProfile.host = "2001:db8::1"
        check("IPv6-адрес SSH принимается", invalidProfile.validationError == nil)
        invalidProfile.host = "router.example\nother"
        check("перевод строки в SSH-узле отклоняется", invalidProfile.validationError != nil)
    }

    func testImportPlanner() throws {
    ...
    func testFailoverProfiles() throws {
    ...
    func testCrypto() throws {
        ...
        } catch { XCTFail("Argon2id: \(error)") }
    }

    func testSecureBackup() throws {
        ...
        do {
            ...
            XCTFail("AES-GCM backup: \(error)")
        }
    }

    func testSettingsCompatibility() throws {
    ...
    func testRCIAuthentication() throws {
