...
extension PingCheckParser {
    ...
    /// Форма записи — та же «плоская», что уже используется в приложении
    /// для WireGuard и object-group: путь целиком в одной команде,
    /// а `no` ставится перед последним звеном.
