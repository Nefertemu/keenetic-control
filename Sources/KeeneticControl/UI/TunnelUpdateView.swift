...
/// Замена конфигурации туннеля: резервная копия, безопасное обновление и
/// откат. Отделено от наблюдения намеренно — это единственный экран
/// приложения, где действия необратимы для туннеля.
struct TunnelUpdateView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                if session.state == nil {
                    ...
                    TunnelInterfacePicker(interfaceIdent: $interfaceIdent, interfaces: interfaces)

                    HStack(spacing: 8) {
                        if hasBaseline {
                            StatusPill(text: "Rollback-база готова", tint: Palette.success,
                                       icon: "checkmark")
                        } else {
                            StatusPill(text: "Rollback-база НЕ задана", tint: Palette.danger,
                                       icon: "exclamationmark")
                        }
                        Text(hasBaseline
                             ? "Есть куда откатиться, если обновление не приживётся."
                             : "Загрузи ТЕКУЩИЙ рабочий .conf и нажми «Сделать rollback-базой» — "
                               + "без этого обновление заблокировано.")
                            .font(.system(size: 11))
        ...
        .onChange(of: session.state?.readAt) { _, _ in
            if interfaceIdent.isEmpty || !interfaces.contains(interfaceIdent) { pickDefault() }
            refreshBaseline()
        }
        .onChange(of: interfaceIdent) { _, _ in refreshBaseline() }
        .onChange(of: session.router.id) { _, _ in
            // Загруженный конфиг относится к старому роутеру — применить его
            // после переключения было бы катастрофой.
            config = nil
            parseError = nil
            pickDefault()
        }
